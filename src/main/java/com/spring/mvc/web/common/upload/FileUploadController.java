...
public class FileUploadController {
    ...
    //업로드 저장 경로
    private static final String UPLOAD_PATH = "D:\\developing_DongWon\\upload";
    ...
    //파일 업로드 요청 처리
    @PostMapping("/upload")
    //MultipartFile : 클라이언트가 전송한 파일의 여러 데이터를 담고 있는 객체
    public String upload(@RequestParam("file") List<MultipartFile> fileList) throws IOException {
        for (MultipartFile file : fileList) {
            ...
            //업로드파일 파일 정보객체 생성
            //file클래스의 생성자를 통해 첫번째 파라리터로 저장경로를
            //두번째 파라미터로 파일명을 넣어 주세요
    ...
    public ResponseEntity<List<String>> uploadAjax(List<MultipartFile> files) throws IOException {
        log.info("비동기 요청" + files.get(0).getOriginalFilename());
...
//                이미지인 경우
                ...
                String encording = new String(fileName.getBytes("UTF-8"), "ISO-8859-1");

                //첨부파일 형식으로 다운로드하도록 헤더에 설정추가
                headers.add("Content-Disposition", "attachment; filename=\"" + encording + "\"");
            }
            //4. 파일데이터를 바이트배열 형식으로 클라이언트에 전송
    ...
    //URI: /deleteFile?fileName=/2021/06/09/sadssad_abc.jpg
    ...
    public ResponseEntity<String> deleteFile(String fileName) throws IOException {
        ...
        try {
            ...
            if (isImage(fileName)) {
                //원본을 지우는 코드
                //fileName => 썸네일 이미지 경로: /2021/06/09/s_asdsadasd_asd.jpg
                //originalName => 원본이미지 경로: /2021/06/09/asdsadasd_asd.jpg

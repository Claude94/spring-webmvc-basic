            ...
            $(function () {
                ...
                $idInput.on('keyup', e => {
                    if ($idInput.val().trim() === '') {
                        ...
                        $('#idChk').html('<b style="color:red; font-size:14px;">[아이디는 필수 정보]</b>');
                        chk1 = false;
                        //아이디를 정규표현식 채턴대로 입력하지 않을 경우
                        //TEST메서드는 정규표현식을 검증하여 값이 일치하면 true,
                        //일치하지 않으면 false를 리턴

                    } else if (!getIdCheck.test($idInput.val())) {
                        $idInput.css('background', 'yellow');
                        $('#idChk').html('<b style="color:red; font-size:14px;">[영문 숫자 4~14자로 작성]</b>');
                        chk1 = false;
                    } else {
                        //아이디 중복확인 비동기 요청
                        //아이디 중복확인 비동기 요청
                        fetch('/check?type=account&keyword=' + $idInput.val())
                            ...
                            .then(flag => {
                                ...
                                if (flag === 'true') {
                                    ...
                                    $('#idChk').html(
                                        '<b style="color:green; font-size:14px;">[참 잘했어요 화이팅!]</b>'
                                    );
                                    chk1 = true;
                                }
                            });
                        ...
                        // $idInput.css('background', 'aqua');
                        // $('#idChk').html('<b style="color:green; font-size:14px;">[참 잘했어요]</b>');
                    }
                });
